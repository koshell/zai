...
set pkglist (cat "$ZAI_DIR/pacman/pkg_official")

txt_major "Refreshing and updating repo database..."
pacman -Syu --noconfirm --color always

txt_major "Installing official repo packages..."
...
if pacman -S --noconfirm --needed --color always $pkglist
	...
	echo -e "Bulk install failed, switching to individual install...\n"	>> $_log
	for i in $pkglist
		echo '========================================================================' 
...
# Load list of packages to install
set pkglist (cat "$ZAI_DIR/pacman/pkg_official.asdeps")
...
echo -e '\nFinished installing offical packages.'
return